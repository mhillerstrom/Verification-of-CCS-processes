...
add_g(P,Q,D,[],[]).
add_g(P,Q,D,[[P,Q,F,G]|L],[[P,Q,F,[D|G]]|L1]) :-
    add_g(P,Q,D,L,L1).
...
print_action_to_pair(Side,[[A,No]|L]) :-
    tab(25), wstring(Side), wstring("--> "), print_action(A),
    wstring(" ==> "), write(No), nl, !, print_action_to_pair(Side,L).

print_action(tau) :- write(epsilon).
print_action(A) :- write(A).

prettyproperties(F,O) :-
